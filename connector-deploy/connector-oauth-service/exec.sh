...
if [ "$1" != "" ] && [ "$2" != "" ]; then
    ...
    case "$2" in
    build)
        docker build --no-cache -t connector-oauth .
        docker tag connector-oauth asia.gcr.io/connectors-166702/connector-oauth
        gcloud docker -- push asia.gcr.io/connectors-166702/connector-oauth
