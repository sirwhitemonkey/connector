...
echo "  ╚██████╗╚██████╔╝██║ ╚████║██║ ╚████║███████╗╚██████╗   ██║   ╚██████╔╝██║  ██║ "
echo "   ╚═════╝ ╚═════╝ ╚═╝  ╚═══╝╚═╝  ╚═══╝╚══════╝ ╚═════╝   ╚═╝    ╚═════╝ ╚═╝  ╚═╝ "
echo " OAuth Service << SpringBoot >>"

success=0
codebase="$BASE_HOME/Development/connectors-springboot"
codebasesrc="${codebase}/connector-oauth-service/script/codebase.src"
...
if [ $success -eq 0 ]; then
 	...
 	echo "---------------------------------------------------"
	echo "<Connector-OAuth-Service>  compiled successful ..."
	echo "---------------------------------------------------"
	target="${codebase}/connector-oauth-service/docker/connector"
	source="${codebase}/connector-oauth-service/target/connector-oauth-service.war"
	cp $source $target
	target="${codebase}/connector-deploy/connector-oauth-service"
	cp $source $target
