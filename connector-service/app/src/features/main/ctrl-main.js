...
angular.module('connector.controllers')
    ...
    function ($scope, $log, $uibModal, $state, $q, $constants, $window, $mdExpansionPanel, $timeout, $mdSidenav, $spinner, $api) {
        ...
        */
        ...
        $scope.event.sysDigCloud = function() {
            var panel = "panel2";
            if (_isAlreadyOpenPanel(panel)) {
                return;
            }

            $scope.data.header  = "SysDigCloud";
            $scope.data.view  = "sysdigcloud";
            _navigatePanel(panel, $scope.data.view);
        ...
        $scope.event.tools = function() {
            var panel = "panel4";
            if (_isAlreadyOpenPanel(panel)) {
                return;
            }
            $scope.data.server1 = undefined;
            $scope.data.server2 = undefined;

            $scope.data.header  = "Tools";
            $scope.data.view  = "tools";
            _navigatePanel(panel, $scope.data.view);
         };

        $scope.event.team = function() {
            var  panel = "panel5";
            if (_isAlreadyOpenPanel(panel)) {
                return;
            }
            ...
            $scope.data.header  = "Team";
            $scope.data.view  = "team";
